...
@section('content')
    ...
    <x-ui.page-header title="Wochenbewertung · {{ $user->name }} · {{ $format::date($summary['start_date']) }} – {{ $format::date($summary['end_date']) }}" class="page-header-compact">
        <x-slot:actions>
            <x-ui.button :href="route('evaluations.month', [$user, \Carbon\Carbon::parse($summary['start_date'])->format('Y-m')])" variant="secondary">Monat</x-ui.button>
            <x-ui.button :href="route('time.month', [\Carbon\Carbon::parse($summary['start_date'])->format('Y-m')])" variant="secondary">Zeitkonto</x-ui.button>
        </x-slot:actions>
    </x-ui.page-header>
    ...
    <div class="metric-grid section-space">
        ...
        <div class="metric-card">
            ...
            <span class="metric-value metric-value-badge">
                <x-status-badge :label="$summary['traffic_light']" kind="traffic-light" />
            </span>
        </div>
    </div>
