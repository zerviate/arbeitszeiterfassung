...
class TimeExportWebController extends Controller
{
    public function __construct(
        private readonly TimeExportService $timeExportService,
    ) {
    }
    ...
    public function dayCsv(Request $request): StreamedResponse
    {
        ...
        $rows = $this->timeExportService->getDayExportRows($actor, $date);

        return $this->streamCsv("arbeitszeiten_tag_{$date}.csv", $rows);
    }
    ...
    public function monthCsv(Request $request): StreamedResponse
    {
        ...
        $rows = $this->timeExportService->getMonthExportRows($actor, $month);

        return $this->streamCsv("arbeitszeiten_monat_{$month}.csv", $rows);
    }
    ...
    public function dayExcel(Request $request): BinaryFileResponse
    {
        ...
        return Excel::download(new ArrayExport($rows), "arbeitszeiten_tag_{$date}.xlsx");
    }
    ...
    public function monthExcel(Request $request): BinaryFileResponse
    {
        ...
        return Excel::download(new ArrayExport($rows), "arbeitszeiten_monat_{$month}.xlsx");
    }
